...
chrome.runtime.onMessage.addListener(function(request, sender, sendResponse){
    if(request && request.action === 'getIP'){
        var url = request.url || (sender && sender.tab ? sender.tab.url : ''),
            parsed = url ? parse_url(url) : '',
            ip = parsed ? localStorage.getItem(parsed) : undefined,
            mnems = JSON.parse(localStorage.getItem('mnems')) || {},
            mnem = mnems[ip] ? mnems[ip].mnem : undefined,
            color = (mnems[ip] && mnems[ip].color) ? mnems[ip].color : (sips.color && sips.color.defaultColor ? sips.color.defaultColor : undefined);
        sendResponse({ip: ip, mnem: mnem, color: color});
        return; // no async
    }
});
