...
var retryInput = document.getElementById('retry_n');
if(retryInput){ retryInput.value = sips.retrySec || 10; retryInput.addEventListener('change',function(){
    var v=parseInt(this.value,10); if(isNaN(v)||v<5||v>60){this.value=sips.retrySec||10;return;} sips.retrySec=v; updateSips();}); }
var allowPrivate = document.getElementById('allow_private');
if(allowPrivate){ allowPrivate.checked = !!sips.allowPrivate; allowPrivate.addEventListener('click',function(){sips.allowPrivate=!!this.checked; updateSips();}); }
