...
function process_response (ipObj) {
	...
	hovConfig = ipObj;
	if(!ipObj) return;
    if(ipObj.color){ hov.style.backgroundColor = ipObj.color; }
    if(ipObj.visible && !el){ document.body.appendChild(hov);} else if((!ipObj.visible)&& el){ document.body.removeChild(el);} 
...
chrome.extension.onMessage.addListener(function (request, sender, response_func) {
    if(request && request.hasOwnProperty('toggle')){
        hovConfig.visible = !hovConfig.visible;
    }
    if(request && request.hasOwnProperty('visible')){
        hovConfig.visible = !!request.visible;
    }
